...
void CSGParamDlg::OnButtonSnapshot() // [2009-12-28 by jzj]: add
{
	...
	// 显示当前SG对象中的数据 // [2009-12-28 by jzj]: add
	UpdateDataFromSG();
	UpdateData(FALSE);
...
{	
	...
	WritePrivateProfileString("采图参数", "是否分通道", str, m_pGrabber->m_sInitFile);
	str.Format("%d", m_pGrabber->m_nGrabSpeed);
	WritePrivateProfileString("采图参数", "采集速度", str, m_pGrabber->m_sInitFile);
	WritePrivateProfileString("采图参数", "图像文件所在文件夹", m_pGrabber->m_strImageFileFolder, m_pGrabber->m_sInitFile);//[2006-10-17]
	
	//[2006-11-30]
	str.Format("%d", m_pGrabber->m_nImageWidth);
	WritePrivateProfileString("采图参数", "图像宽度", str, m_pGrabber->m_sInitFile);
	str.Format("%d", m_pGrabber->m_nImageHeight);
	WritePrivateProfileString("采图参数", "图像高度", str, m_pGrabber->m_sInitFile);
	str.Format("%d", m_pGrabber->m_nImageByteCount);
	WritePrivateProfileString("采图参数", "每象素字节量", str, m_pGrabber->m_sInitFile); 

// 	str.Format("%d", m_pGrabber->m_strImageFileFolder);
// 	WritePrivateProfileString("采图参数", "图像文件所在文件夹", str, m_pGrabber->m_sInitFile); 
}
